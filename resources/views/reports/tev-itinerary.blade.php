...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── BOTTOM CERTIFICATION + SIGNATURES ── */
    .bottom-wrap {
        width: 100%;
        margin-top: 12px;
        display: table;
...
<table class="header-tbl">
    <tr>
        <td class="logo-left">
            <img src="{{ public_path('assets/img/dole_logo.png') }}" alt="DOLE">
        </td>
        ...
        <td class="logo-right">
            <img src="{{ public_path('assets/img/bagong_pilipinas.png') }}" alt="Bagong Pilipinas">
        </td>
    </tr>
</table>
...
<table class="meta-tbl">
    <tr>
        ...
        <td class="meta-val" style="width:50%; font-weight:bold;">{{ $empName }}</td>
        <td style="width:1%;"></td>
        <td></td>
    </tr>
    <tr>
        <td class="meta-lbl">Position:</td>
        <td class="meta-val">{{ optional($emp)->position_title ?? '—' }}</td>
    </tr>
    <tr>
        <td class="meta-lbl">Official Station:</td>
        <td class="meta-val">{{ optional($emp)->official_station ?? 'DOLE 9 - TSSD, ZAMBOANGA CITY' }}</td>
    </tr>
    <tr>
        <td class="meta-lbl">Purpose:</td>
        <td class="meta-val" colspan="3">{{ $tev->purpose }}</td>
    </tr>
    @if ($tev->destination)
    <tr>
        <td class="meta-lbl"></td>
        <td class="meta-val" colspan="3">{{ $tev->destination }}</td>
    </tr>
    @endif
</table>
...
<table class="itin-tbl">
    <thead>
        <tr>
            ...
            <th rowspan="2" style="width:72px;">Means of<br>Transportation</th>
            <th colspan="2">Allowable Expenses</th>
            <th rowspan="2" style="width:65px;">Total<br>Amount</th>
        </tr>
        <tr>
            <th style="width:100px;">From Destination</th>
            <th style="width:100px;">To Destination</th>
            <th style="width:52px;">Departure</th>
            <th style="width:52px;">Arrival</th>
            <th style="width:70px;">Transportation</th>
            <th style="width:70px;">Allowance</th>
        </tr>
    </thead>
    <tbody>
        @forelse ($lines as $line)
            ...
            <tr>
                ...
                <td>{{ $line->departure_time
                        ? \Carbon\Carbon::parse($line->departure_time)->format('h:iA')
                        : '' }}</td>
                <td>{{ $line->arrival_time
                        ? \Carbon\Carbon::parse($line->arrival_time)->format('h:iA')
                        : '' }}</td>
                <td>{{ strtoupper($line->mode_of_transport ?? '') }}</td>
                <td class="td-r">
                    {{ (float)$line->transportation_cost > 0
                        ? number_format($line->transportation_cost, 2)
                        : '' }}
                </td>
                <td class="td-r">
                    {{ (float)$line->per_diem_amount > 0
                        ? number_format($line->per_diem_amount, 2)
                        : '' }}
                </td>
                <td class="td-r">
        ...
        @empty
            <tr>
                <td colspan="9" style="text-align:center; padding:10px; font-style:italic; color:#666;">
                    No itinerary lines.
                </td>
            </tr>
        @endforelse
...
<div class="bottom-wrap" style="margin-top:16px;">
